...
PixelShaderOutput PS(VertexShaderOutput input)
{
	...
	float3 normal_sampler = tex2D(uniform_normal_sampler, input.texcoord).xyz;
	...
	// アルベド
	float3 albedo = tex2D(uniform_albedo_sampler, input.texcoord).xyz + (diffuse_cube_ambient.xyz*0.5f);
